<template>
    <div class='flex justify-center w-full'>
        <div v-if="isDataLoaded">
            ...
            <div v-else class="">
                <ul>
                    <li v-for="(follower, index) in followers" :key="followers.id" class="mb-1">
                        <div class="p-2 bg-neutral-400 rounded-xl inline-flex items-center w-full justify-between">
                            <div class="flex items-center mr-3">
                                <img v-if="follower.image_url" :src="follower.image_url" alt="User Image"
                                ...
                                <div class="ml-2 p-2 hover:cursor-pointer hover:shadow-inner hover:shadow-neutral-500 hover:rounded-2xl hover:scale-95 transition-transform duration-200 ease-in-out" @click="open_homePage(follower)">
                                    {{ follower.login || (follower.first_name + ' ' + follower.second_name) }}
                                </div>
                            </div>
                            ...
                            <div ref="checkDropdown" class="relative dropdown flex-inline">
                                <div class="flex items-center shadow-lg rounded-xl bg-cyan-600 text-white p-1 mainButton w-48 justify-between">
                                    ...
                                    <button id='dropdownButton' type='button' @click="toggleDropdown(index)" class="dropdownActions hover:scale-90 transition-transform">
                                ...
                                <div v-if="follower.showOptions" class="absolute bg-cyan-600 shadow-lg rounded-b-xl p-2 z-10 w-full">
                                    <button @click="addFriend(follower.id, index)" class="flex items-center rounded-full text-green-400 p-2 hover:bg-cyan-700 hover:scale-95 transition-transform duration-300 ease-in-out w-full">
                                    ...
                                    <button @click="blockUser(follower.id, index)" class="flex items-center rounded-full text-red-400 p-2 hover:bg-cyan-700 hover:scale-95 transition-transform duration-300 ease-in-out w-full">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
...
export default {
    ...
    setup() {
        ...
        const addFriend = async(id, index) => {
            try {
                let userName = followers.value[index].login || followers.value[index].first_name + ' ' + followers.value[index].second_name;
                let resultAlert = await showAlertWindow("Подтверждение", "Добавить в друзья пользователя " + userName + '?');
                if (resultAlert === true) {
                    await axios.post(`/friendship/friends/${id}`);
                    await loadFollowers();
                }
                hideMainButton();
            } catch (e) {
        ...
        const handleClickOutside = (event) => {
            if (!event.target.closest('.relative.dropdown')) {
                followers.value.forEach(user => user.showOptions = false);
