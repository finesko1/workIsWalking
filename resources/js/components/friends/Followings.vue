<template>
    <div class='flex justify-center w-full'>
        <div v-if="isDataLoaded">
            ...
            <div v-else class="">
            <ul>
                <li v-for="(following, index) in followings" :key="following.id" class="mb-1">
                    <div class="p-2 bg-neutral-400 rounded-xl inline-flex items-center w-full justify-between">
                        <div class="flex items-center mr-3">
                            <img v-if="following.image_url" :src="following.image_url" alt="User Image"
                            ...
                            <div class="ml-2 p-2 hover:cursor-pointer hover:shadow-inner hover:shadow-neutral-500 hover:rounded-2xl hover:scale-95 transition-transform duration-200 ease-in-out" @click="open_homePage(following)">
                                {{ following.login || (following.first_name + ' ' + following.second_name) }}
                            </div>
                        </div>
                        ...
                        <div ref="checkDropdown" class="relative dropdown flex-inline">
                            ...
                            <div v-if="following.showOptions" class="absolute bg-cyan-600 shadow-lg rounded-b-xl p-2 z-10 w-full">
                                <button @click="cancelFollowing(following.id, index)" class="flex items-center rounded-full text-red-400 p-2 hover:bg-cyan-700 hover:scale-95 transition-transform duration-300 ease-in-out w-full">
                                ...
                                <button @click="blockUser(following.id, index)" class="flex items-center rounded-full text-red-400 p-2 hover:bg-cyan-700 hover:scale-95 transition-transform duration-300 ease-in-out w-full">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
...
export default {
    ...
    setup() {
        ...
        const cancelFollowing = async (userId, index) => {
            try {
                let userName = followings.value[index].login || followings.value[index].first_name + ' ' + followings.value[index].second_name;
        ...
        const handleClickOutside = (event) => {
            if (!event.target.closest('.relative.dropdown')) {
                followings.value.forEach(user => user.showOptions = false);
