                        ...
                        <div ref="checkDropdown" class="relative dropdown">
                            <div class="flex items-center shadow-lg rounded-xl bg-cyan-600 text-white p-1 mainButton w-48 justify-between">
                                <div class="text-white">
                                    <div v-if="user.status === 'accepted'">В друзьях</div>
                                    <div v-else-if="user.status === null" @click="addFollowing(user.id)"
                                        class="hover:scale-95 hover:cursor-pointer transition-transform duration-200 ease-in-out">
                                        Отправить заявку
                                    </div>
                                    <div v-else-if="user.status === 'follower'" class="text-blue">Подписан на вас</div>
                                    <div v-else-if="user.status === 'following'" class="text-white">Заявка отправлена</div>
                                    <div v-else-if="user.status === 'pending'">Ожидает ответ</div>
                                    <div v-else-if="user.status === 'blockIt'">Заблокирован</div>
                                    <div v-else-if="user.status === 'blockMe'" class="">Вы в черном списке</div>
                                    <div v-else-if="user.status === 'blocked'">Заблокирован</div>
                                </div>
...
<!--                                Вы подписаны на него-->
                                <div v-else-if="user.status === 'following'" class="w-full">
                                    <button @click="cancelFollowing(user.id)" class="flex items-center rounded-full text-red-400 p-2 hover:bg-cyan-700 hover:scale-95 transition-transform duration-300 ease-in-out w-full">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
