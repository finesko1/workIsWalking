<template>
    <div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-60 h-full">
        <div class="flex-col p-6 bg-white rounded-xl min-w-min relative">
            ...
            <div class="p-4">
                <div class="p-2 bg-neutral-400 rounded-xl inline-flex items-center w-full justify-between">
                    <div class="flex items-center mr-3">
                        ...
                        <div class="flex justify-between space-x-2 mx-2 flex-wrap items-center">
                            <div class="flex">
                                <select v-model="user.role" @change="updateUserRole" class="p-2 rounded bg-white border">
                                    <option value="admin">Руководитель</option>
                                    <option value="moderator">Помощник</option>
        ...
        setup (props, {emit}) {
            ...
            const updateUserRole = async () => {
                try {
                    ...
                    formData.append('newRole', user.value.role)
                    console.log(`User role updated to: ${user.value.role}`);
                    await axios.post(`/group/${groupId.value}/${user.value.id}/updateRole`, formData)
                } catch (e) {
